...
export async function writeMemory(params: {
  ...
  for (const tag of memory.tags) {
    await redis.sadd(tagKey(tag), id);
  }
...
export async function listMemories(params: {
  ...
  if (params.layer) {
    ...
    const sets = await Promise.all(
      params.tags.map(tag => redis.smembers(tagKey(tag)) as Promise<string[]>)
    );
    ...
    const allSets = await Promise.all(
      layers.map(l => redis.smembers(layerKey(l)) as Promise<string[]>)
    );
    ids = allSets.flat();
  }
...
export async function updateMemory(
  ...
  if (updates.tags) {
    for (const tag of existing.tags) {
      await redis.srem(tagKey(tag), id);
    }
...
export async function deleteMemory(id: string): Promise<boolean> {
  ...
  await redis.del(memKey(id));
  await redis.srem(layerKey(existing.layer), id);
  for (const tag of existing.tags) {
    await redis.srem(tagKey(tag), id);
  }
  return true;
...
export async function getStats(): Promise<MemoryStats> {
  ...
  const counts = await Promise.all(
    layers.map(async l => {
      const ids = (await redis.smembers(layerKey(l))) as string[];
      return ids.length;
    })
  );
...
export async function cleanExpiredDaily(): Promise<number> {
  ...
  for (const id of ids) {
    const raw = await redis.get<string>(memKey(id));
    if (!raw) {
      await redis.srem(layerKey('daily'), id);
      cleaned++;
      continue;
    }
    const mem: Memory = typeof raw === 'string' ? JSON.parse(raw) : raw;
