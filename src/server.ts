...
export function createServer(): McpServer {
  ...
  server.registerTool(
    ...
    async ({ content, layer, tags, source }) => {
      const mem = await writeMemory({ content, layer: layer as MemoryLayer, tags, source });
  ...
  server.registerTool(
    ...
    async ({ layer, tags, keyword, limit }) => {
      const memories = await listMemories({ layer: layer as MemoryLayer | undefined, tags, keyword, limit });
      return {
        content: [{
          type: 'text' as const,
          text: memories.length === 0 ? '没有找到符合条件的记忆。' : JSON.stringify({ memories, count: memories.length }, null, 2),
        }],
      };
    }
  );
  ...
  server.registerTool(
    ...
    async ({ query, layer, limit }) => {
      const memories = await listMemories({ layer: layer as MemoryLayer | undefined, keyword: query, limit });
      return {
        content: [{
          type: 'text' as const,
          text: memories.length === 0 ? `没有找到包含"${query}"的记忆。` : JSON.stringify({ memories, query, count: memories.length }, null, 2),
        }],
      };
    }
  );
  ...
  server.registerTool(
    ...
    async ({ id, content, tags, source }) => {
      ...
      return {
        content: [{
          type: 'text' as const,
          text: updated ? JSON.stringify(updated, null, 2) : `未找到ID为 ${id} 的记忆。`,
        }],
      };
    }
  );
